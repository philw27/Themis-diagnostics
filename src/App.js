    ...
    <div style={{minHeight:"85vh",background:`linear-gradient(160deg,#060a10,#0d1a2a,#060e1a)`,borderRadius:16,padding:28,display:"flex",flexDirection:"column",justifyContent:"space-between",border:`1px solid ${C.border}`}}>
      ...
      <div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
          {[[c2s.length,"C2",C.red],[c3s.length,"C3",C.amber],[fis.length,"FI",C.purple]].map(([n,l,col])=>(
            <div key={l} style={{background:col+"22",border:`1px solid ${col}44`,borderRadius:8,padding:"12px",textAlign:"center"}}>
      ...
      {Object.entries(CHECKLIST_ITEMS).map(([secId,items])=>{
        ...
        return (
          <div key={secId} style={{...S.card,marginBottom:10}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
              <div style={{fontSize:12,fontWeight:700,color:C.accent}}>{label}</div>
              <div style={{display:"flex",gap:6}}>
                {flagged>0&&<span style={S.tag(C.red)}>{flagged} flagged</span>}
    ...
    <div style={{padding:4}}>
      ...
      {review?.risk_items?.length>0?(
        review.risk_items.map((r,i)=>{
          ...
          return (
            <div key={i} style={{...S.card,borderLeft:`4px solid ${col}`,marginBottom:10}}>
    ...
    return (
      <div style={{padding:16}}>
        ...
        <div style={{...S.card,border:`2px solid ${statusCol}44`,marginBottom:14}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:14}}>
            {[[c2s.length,"C2",C.red],[c3s.length,"C3",C.amber],[fis.length,"FI",C.purple]].map(([n,l,col])=>(
              <div key={l} style={{background:col+"22",border:`1px solid ${col}44`,borderRadius:8,padding:"10px",textAlign:"center"}}>
  ...
  return (
    <div style={{paddingBottom:40}}>
      ...
      <div style={{...S.header,position:"sticky",top:0,zIndex:100}}>
        <button onClick={()=>setPage(p=>Math.max(0,p-1))} style={{background:"none",border:`1px solid ${C.border}`,color:C.text,borderRadius:8,padding:"6px 12px",fontSize:13,cursor:"pointer",fontFamily:"inherit"}}>
          {page===1?"✕ Close":"← Prev"}
        </button>
      ...
      <div style={{display:"flex",justifyContent:"center",gap:6,padding:"8px 0"}}>
        {pages.map((_,i)=>(
          <div key={i} onClick={()=>setPage(i+1)} style={{width:i+1===page?20:6,height:6,borderRadius:3,background:i+1===page?C.green:C.border,cursor:"pointer",transition:"all 0.2s"}}/>
        ))}
      </div>
    </div>
  );
      ...
      {screen!=="login" && (
        <div style={S.header}>
          ...
          <div style={{display:"flex",gap:10,alignItems:"center"}}>
            ...
            {labels[screen] && <span style={{fontSize:10,color:C.muted,letterSpacing:"0.1em"}}>{labels[screen].toUpperCase()}</span>}
            <button onClick={handleLogout} style={{background:"none",border:`1px solid ${C.border}`,color:C.muted,borderRadius:6,padding:"4px 10px",fontSize:10,cursor:"pointer",fontFamily:"inherit"}}>OUT</button>
          </div>
        </div>
      )}
      ...
      <div style={{paddingBottom:40}}>
        ...
        {screen==="dashboard" && (
          <Dashboard
            ...
            onSelectJob={j=>{
              setJob(j);
              // Load demo data for the demo job
              if (j.id===DEMO_JOB.id||j.id===2) {
                setAsset(DEMO_ASSET);
                setChecklist(DEMO_CHECKLIST);
                setTestResults(DEMO_TEST_RESULTS);
                const flagged = Object.entries(DEMO_CHECKLIST)
                  .filter(([,v])=>["no","lim","fi"].includes(v.answer))
                  .map(([k,v])=>({id:k,...v}));
                setReview(runAnalysis(j,DEMO_ASSET,DEMO_CHECKLIST,DEMO_TEST_RESULTS,flagged));
                setScreen("summary");
              } else {
