...
function generateEICRPDF(job, eicrInstall, eicrSupply, eicrCircuits, eicrInspection, eicrObservations, profile) {
  try {
  const doc = new jsPDF({ orientation: "portrait", unit: "mm", format: "a4" });
  const W = 210, H = 297, margin = 14, contentW = 182;

  const NAVY = [30, 58, 95], GREEN = [5, 150, 105], RED = [220, 38, 38];
  const AMBER = [217, 119, 6], PURPLE = [124, 58, 237];
  const LGREY = [248, 250, 252], MGREY = [226, 232, 240];
  const TEXT = [30, 41, 59], MUTED = [100, 116, 139];
  ...
  const selectedAgreed = agreedIds.map(i => ({ letter: String.fromCharCode(97 + i), text: RIVERSIDE_AGREED[i] })).filter(x => x.text);
  const selectedOp = opIds.map(i => ({ num: i + 1, text: RIVERSIDE_OP[i] })).filter(x => x.text);

  let pageNum = 0;

  const drawHeaderFooter = (isLandscape) => {
    ...
    doc.text("ELECTRICAL INSTALLATION CONDITION REPORT", 14, 7.5);
    doc.text(`${job?.jobNumber || ""} · ${job?.client || ""} · ${job?.address || ""}`, pw - 14, 7.5, { align: "right" });
    doc.setFillColor(...MGREY);
    doc.rect(0, ph - 9, pw, 9, "F");
    doc.setFontSize(6.5);
    doc.setTextColor(...MUTED);
    doc.text("Themis Diagnostics · Elect Building & Maintenance Ltd", 14, ph - 3.5);
    doc.text(`Page ${pageNum}`, pw / 2, ph - 3.5, { align: "center" });
    doc.text("BS 7671:2018+A2:2022", pw - 14, ph - 3.5, { align: "right" });
  };

  const newPage = (landscape) => {
    if (pageNum > 0) {
      if (landscape) doc.addPage([297, 210]);
      else doc.addPage();
    }
    pageNum++;
    drawHeaderFooter(landscape);
    return landscape ? 14 : 14;
  };

  const secHeader = (y, text, color) => {
  ...
  const wrapText = (y, text, size, color, lineH, maxW) => {
    doc.setFontSize(size);
    doc.setTextColor(...color);
    const lines = doc.splitTextToSize(String(text || ""), maxW || contentW);
    lines.forEach((line, i) => doc.text(line, margin, y + i * lineH));
    return y + lines.length * lineH;
  };

  const tbl = (startY, body, colStyles, opts) => {
    autoTable(doc, {
      startY,
      margin: { left: margin, right: margin },
      tableWidth: contentW,
      theme: "plain",
      styles: { fontSize: 7.5, cellPadding: 2.5, lineColor: MGREY, lineWidth: 0.3 },
      ...opts,
      body,
  ...
  doc.setFontSize(15);
  doc.setTextColor(255, 255, 255);
  doc.text("Electrical Installation Condition Report", margin + 5, y + 11);
  doc.setFontSize(8);
  doc.setTextColor(200, 220, 255);
  doc.text("BS 7671:2018+A2:2022 · IET Wiring Regulations 18th Edition", margin + 5, y + 19);
  // Status badge
  doc.setFillColor(...(overall === "SATISFACTORY" ? GREEN : RED));
  doc.roundedRect(W - margin - 44, y + 7, 42, 12, 2, 2, "F");
  doc.setFontSize(8);
  doc.setTextColor(255, 255, 255);
  doc.text(overall, W - margin - 23, y + 15, { align: "center" });
  y += 30;

  // Section 1 - Client
  y = secHeader(y, "1  CLIENT DETAILS");
  y = tbl(y, [
    [{ content: "Client / Occupier", styles: { textColor: MUTED, fontSize: 6.5 } }, { content: "Address of Installation", styles: { textColor: MUTED, fontSize: 6.5 } }],
    [front.occupier_title || job?.client || "—", job?.address || "—"],
    [{ content: "Job Reference", styles: { textColor: MUTED, fontSize: 6.5 } }, { content: "Date of Inspection", styles: { textColor: MUTED, fontSize: 6.5 } }],
    [job?.jobNumber || "—", job?.date || "—"],
  ], { 0: { cellWidth: 91 }, 1: { cellWidth: 91 } });

  // Section 2 - Reason
  y = secHeader(y, "2  REASON FOR PRODUCING THIS REPORT");
  y += 3;
  y = wrapText(y, front.reason || "Periodic inspection and testing of the electrical installation.", 8, TEXT, 4.5) + 4;

  // Section 3 - Installation details
  y = secHeader(y, "3  DETAILS OF THE INSTALLATION");
  y = tbl(y, [
    ["Estimated age of wiring (yrs)", front.wiring_age || "—", "Evidence of additions/alterations", front.evidence_additions ? "Yes" : "No"],
    ["Date of last inspection", front.last_inspection || "—", "Records available (Reg 651.1)", front.records_available || "N/A"],
  ], { 0: { cellWidth: 58, textColor: MUTED, fontSize: 6.5 }, 1: { cellWidth: 33 }, 2: { cellWidth: 58, textColor: MUTED, fontSize: 6.5 }, 3: { cellWidth: 33 } });

  // Section 4 - Extent & Limitations
  y = secHeader(y, "4  EXTENT AND LIMITATIONS OF INSPECTION AND TESTING");
  y += 3;
  doc.setFontSize(6.5); doc.setTextColor(...MUTED); doc.text("Purpose:", margin, y);
  y = wrapText(y + 4, front.purpose || "Periodic inspection and testing of the fixed electrical installation.", 7.5, TEXT, 4.2) + 3;
  doc.setFontSize(6.5); doc.setTextColor(...MUTED); doc.text("Extent:", margin, y);
  y = wrapText(y + 4, front.extent || "This report covers the inspection and testing of the fixed electrical wiring system within the named property, with the exception of any agreed or operational limitations as documented.", 7.5, TEXT, 4.2) + 3;

  if (agreedIds.length > 0 || opIds.length > 0) {
    doc.setFillColor(240, 249, 255);
    const limH = (agreedIds.length > 0 ? 6 : 0) + (opIds.length > 0 ? 6 : 0) + 4;
    doc.roundedRect(margin, y, contentW, limH, 1, 1, "F");
    doc.setFontSize(7.5); doc.setTextColor(...NAVY);
    let ly = y + 5;
    if (agreedIds.length > 0) { doc.text(`Agreed limitations: ${agreedIds.map(i => String.fromCharCode(97 + i)).join(", ")} — please refer to continuation sheet`, margin + 3, ly); ly += 6; }
    if (opIds.length > 0) { doc.text(`Operational limitations: ${opIds.map(i => i + 1).join(", ")} — please refer to continuation sheet`, margin + 3, ly); }
    y += limH + 3;
  }

  doc.setFontSize(7); doc.setTextColor(...MUTED); doc.text("Agreed with:", margin, y + 3);
  doc.setTextColor(...TEXT); doc.text(String(front.agreed_with || "—"), margin + 26, y + 3);
  y += 8;

  // Section 5 - Overall condition
  y = secHeader(y, "5  OVERALL CONDITION OF THE INSTALLATION");
  doc.setFillColor(...(overall === "SATISFACTORY" ? [240, 253, 244] : [254, 242, 242]));
  doc.roundedRect(margin, y, contentW, 12, 1, 1, "F");
  doc.setFontSize(11); doc.setTextColor(...(overall === "SATISFACTORY" ? GREEN : RED));
  doc.text(overall, W / 2, y + 8, { align: "center" });
  y += 14;
  doc.setFontSize(7); doc.setTextColor(...MUTED); doc.text("Next inspection:", margin, y + 3);
  doc.setTextColor(...TEXT); doc.text(String(front.next_inspection || "—"), margin + 28, y + 3);
  y += 8;

  // Section 6 - Findings summary
  y = secHeader(y, "6  SUMMARY OF FINDINGS");
  y = tbl(y, [
    [
      { content: "C1\nDanger Present", styles: { textColor: RED, fontStyle: "bold", fontSize: 6.5, halign: "center" } },
      { content: "C2\nPotentially Dangerous", styles: { textColor: ORANGE, fontStyle: "bold", fontSize: 6.5, halign: "center" } },
      { content: "C3\nImprovement Recommended", styles: { textColor: AMBER, fontStyle: "bold", fontSize: 6.5, halign: "center" } },
      { content: "FI\nFurther Investigation", styles: { textColor: PURPLE, fontStyle: "bold", fontSize: 6.5, halign: "center" } },
    ],
    [
      { content: String(c1s.length), styles: { textColor: RED, fontSize: 16, fontStyle: "bold", halign: "center" } },
      { content: String(c2s.length), styles: { textColor: ORANGE, fontSize: 16, fontStyle: "bold", halign: "center" } },
      { content: String(c3s.length), styles: { textColor: AMBER, fontSize: 16, fontStyle: "bold", halign: "center" } },
      { content: String(fis.length), styles: { textColor: PURPLE, fontSize: 16, fontStyle: "bold", halign: "center" } },
    ],
  ], { 0: { cellWidth: 45 }, 1: { cellWidth: 45 }, 2: { cellWidth: 46 }, 3: { cellWidth: 46 } });

  // Engineer details on cover
  y = secHeader(y, "INSPECTING ENGINEER");
  y = tbl(y, [
  ...
  ], { 0: { cellWidth: 35, textColor: MUTED, fontSize: 6.5 }, 1: { cellWidth: 56 }, 2: { cellWidth: 35, textColor: MUTED, fontSize: 6.5 }, 3: { cellWidth: 56 } });

  // ── PAGE 2: SUPPLY CHARACTERISTICS ───────────────────────
  y = newPage(false);
  y = secHeader(y, "10  SUPPLY CHARACTERISTICS AND EARTHING ARRANGEMENTS");
  y = tbl(y, [
    ["Earthing arrangement", [supply.earthing_tncs && "TN-C-S", supply.earthing_tns && "TN-S", supply.earthing_tt && "TT"].filter(Boolean).join(", ") || "—", "Live conductors", [supply.phases_1 && "1-phase (2-wire)", supply.phases_3_4w && "3-phase (4-wire)"].filter(Boolean).join(", ") || "—"],
    ["Nominal voltage UO (V)", String(supply.voltage_uo || "230"), "Frequency (Hz)", String(supply.frequency || "50")],
    ["Prospective fault current Ipf", String(supply.prospective_fault_current || "—"), "External Ze (Ω)", String(supply.external_ze || "—")],
  ], { 0: { cellWidth: 55, textColor: MUTED, fontSize: 6.5 }, 1: { cellWidth: 36 }, 2: { cellWidth: 55, textColor: MUTED, fontSize: 6.5 }, 3: { cellWidth: 36 } });

  y = secHeader(y, "11  PARTICULARS OF INSTALLATION");
  y = tbl(y, [
    [{ content: "MEANS OF EARTHING", colSpan: 4, styles: { fillColor: LGREY, textColor: MUTED, fontSize: 6.5, fontStyle: "bold" } }],
    ["Distributor facility", supply.distributor_facility ? "Yes" : "No", "Earth electrode", supply.installation_electrode ? "Yes" : "No"],
    ["Earth conductor material", String(supply.earth_conductor_material || "Copper"), "Earth conductor CSA (mm²)", String(supply.earth_conductor_csa || "—")],
    [{ content: "MAIN PROTECTIVE BONDING", colSpan: 4, styles: { fillColor: LGREY, textColor: MUTED, fontSize: 6.5, fontStyle: "bold" } }],
    ["Bonding material", String(supply.bonding_conductor_material || "Copper"), "Bonding CSA (mm²)", String(supply.bonding_conductor_csa || "—")],
    ["Services bonded", [supply.bond_water && "Water", supply.bond_gas && "Gas", supply.bond_oil && "Oil"].filter(Boolean).join(", ") || "None", "", ""],
    [{ content: "MAIN SWITCH / CIRCUIT BREAKER", colSpan: 4, styles: { fillColor: LGREY, textColor: MUTED, fontSize: 6.5, fontStyle: "bold" } }],
    ["Location", String(supply.main_switch_location || "—"), "BS number", String(supply.main_switch_bs || "—")],
    ["Poles", String(supply.main_switch_poles || "—"), "Current rating (A)", String(supply.main_switch_rating || "—")],
    ["Voltage rating (V)", String(supply.main_switch_voltage || "—"), "RCD main switch", supply.rcd_main ? "Yes" : "No"],
  ], { 0: { cellWidth: 55, textColor: MUTED, fontSize: 6.5 }, 1: { cellWidth: 36 }, 2: { cellWidth: 55, textColor: MUTED, fontSize: 6.5 }, 3: { cellWidth: 36 } });

  y = secHeader(y, "TEST INSTRUMENTS USED");
  y = tbl(y, [
    ["Multifunction tester (serial/cal)", String(supply.test_mft || "—"), "Insulation resistance (serial/cal)", String(supply.test_ir || "—")],
    ["Continuity tester (serial/cal)", String(supply.test_continuity || "—"), "Loop impedance (serial/cal)", String(supply.test_loop || "—")],
    ["RCD tester (serial/cal)", String(supply.test_rcd || "—"), "Earth electrode (serial/cal)", String(supply.test_electrode || "—")],
  ], { 0: { cellWidth: 55, textColor: MUTED, fontSize: 6.5 }, 1: { cellWidth: 36 }, 2: { cellWidth: 55, textColor: MUTED, fontSize: 6.5 }, 3: { cellWidth: 36 } });

  // ── PAGE 3: CIRCUIT SCHEDULE (LANDSCAPE) ─────────────────
  pageNum++;
  doc.addPage([297, 210]);
  ...
  autoTable(doc, {
    ...
    tableWidth: lCW,
    theme: "striped",
    styles: { fontSize: 5.5, cellPadding: 1.5, lineColor: MGREY, lineWidth: 0.2, halign: "center", overflow: "ellipsize" },
    headStyles: { fillColor: NAVY, textColor: [255, 255, 255], fontSize: 5.5, fontStyle: "bold" },
    head: [["Ref", "Designation", "Wiring\nType", "Ref\nMethod", "Points", "CB\nRating", "CB\nType", "CB\nBS No", "RCD\nType", "RCD\nmA", "Max Zs\nΩ", "R1+R2\nΩ", "R2\nΩ", "Rn\nΩ", "IR\nL-L MΩ", "IR\nL-E MΩ", "Polarity", "Zs\nMeas.", "RCD\nms", "AFD"]],
    body: circuits.length > 0 ? circuits.map(c => [
      c.circuit_ref||"", c.designation||"", c.wiring_type||"", c.ref_method||"",
      c.points||"", c.cb_rating||"", c.cb_type||"", c.cb_bs||"",
      c.rcd_type||"", c.rcd_rating||"", c.max_zs||"", c.r1_r2||"",
      c.r2||"", c.rn||"", c.ir_ll||"", c.ir_le||"",
      c.polarity||"", c.zs_measured||"", c.rcd_op_time||"", c.afd_test||"",
    ]) : [["—", "No circuits recorded", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", ""]],
  });

  // ── PAGE 4: INSPECTION SCHEDULE ───────────────────────────
  y = newPage(false);
  y = secHeader(y, "12  SCHEDULE OF ITEMS INSPECTED");

  const ansLabel = (a) => ({ pass: "✓", c1: "C1", c2: "C2", c3: "C3", fi: "FI", lim: "LIM", nv: "N/V", na: "N/A" }[a] || a || "—");
  const ansCol = (a) => ({ pass: GREEN, c1: RED, c2: ORANGE, c3: AMBER, fi: PURPLE, lim: [2, 132, 199] }[a] || MUTED);

  const inspRows = Object.entries(inspection).map(([id, v]) => [
    id,
    id,
    { content: ansLabel(v.answer), styles: { textColor: ansCol(v.answer), fontStyle: "bold", halign: "center" } },
    v.note || "",
  ]);
  ...
  if (inspRows.length > 0) {
    autoTable(doc, {
      ...
      head: [["Item Ref", "Description", "Outcome", "Notes"]],
      body: inspRows,
      columnStyles: { 0: { cellWidth: 18 }, 1: { cellWidth: 105 }, 2: { cellWidth: 22, halign: "center" }, 3: { cellWidth: 37 } },
    });
    y = doc.lastAutoTable.finalY + 4;
  } else {
    y += 5;
    doc.setFontSize(8); doc.setTextColor(...MUTED);
    doc.text("No inspection schedule items recorded.", margin, y);
    y += 10;
  }

  // ── PAGE 5: OBSERVATIONS ─────────────────────────────────
  y = newPage(false);
  y = secHeader(y, "7  OBSERVATIONS");
  ...
  if (obs.length === 0) {
    ...
    autoTable(doc, {
      ...
      body: obs.map(o => [
        { content: o.code || "—", styles: { textColor: ({ C1: RED, C2: ORANGE, C3: AMBER, FI: PURPLE }[o.code] || TEXT), fontStyle: "bold", halign: "center" } },
        o.description || "—", o.regulation || "—", o.location || "—", o.recommendation || "—",
      ]),
      columnStyles: { 0: { cellWidth: 13 }, 1: { cellWidth: 58 }, 2: { cellWidth: 27 }, 3: { cellWidth: 30 }, 4: { cellWidth: 54 } },
    });
  }
  ...
  if (selectedAgreed.length > 0 || selectedOp.length > 0) {
    ...
    doc.text("Continuation Sheet — Limitations", margin, y + 6);
    y += 14;

    if (selectedAgreed.length > 0) {
      y = secHeader(y, "AGREED LIMITATIONS");
      y += 5;
      selectedAgreed.forEach(({ letter, text }) => {
        doc.setFontSize(8); doc.setTextColor(...NAVY);
        doc.text(`${letter})`, margin, y + 4);
        y = wrapText(y, text, 7.5, TEXT, 4.2, contentW - 10) + 5;
        y += 2;
      });
      y += 4;
    }
    ...
    if (selectedOp.length > 0) {
      y = secHeader(y, "OPERATIONAL LIMITATIONS");
      y += 5;
      selectedOp.forEach(({ num, text }) => {
        doc.setFontSize(8); doc.setTextColor(...NAVY);
        doc.text(`${num})`, margin, y + 4);
        y = wrapText(y, text, 7.5, TEXT, 4.2, contentW - 10) + 5;
        y += 2;
      });
    }
  ...
  // ── PAGE 7: DECLARATION ───────────────────────────────────
  y = newPage(false);
  y = secHeader(y, "DECLARATION");
  y += 5;
  y = wrapText(y, "I/We, being the person(s) responsible for the inspection and testing of the electrical installation (as indicated by my/our signatures below), particulars of which are described above, having exercised reasonable skill and care when carrying out the inspection and testing, hereby declare that the information in this report, including the observations and the attached schedules, provides an accurate assessment of the condition of the electrical installation taking into account the agreed limitations detailed in section 4.", 8, TEXT, 4.5) + 8;

  y = secHeader(y, "ENGINEER SIGNATURE");
  y = tbl(y, [
    ["Name", profile?.full_name || job?.engineer || "—", "Qualification", profile?.qualification || "—"],
    ["Reg. Number", profile?.reg_number || "—", "Company", profile?.company || "Elect Building & Maintenance Ltd"],
    ["Date of inspection", job?.date || "—", "Signature", ""],
  ], { 0: { cellWidth: 35, textColor: MUTED, fontSize: 6.5 }, 1: { cellWidth: 56 }, 2: { cellWidth: 35, textColor: MUTED, fontSize: 6.5 }, 3: { cellWidth: 56 } });

  if (profile?.signature_data) {
    try { doc.addImage(profile.signature_data, "PNG", margin + 92, y - 14, 50, 11); } catch(e) {}
  }

  y += 6;
  y = secHeader(y, "FOR AND ON BEHALF OF CLIENT — RECEIVED BY");
  tbl(y, [
    ["Name", "", "Position", ""],
    ["Date received", "", "Signature", ""],
  ], { 0: { cellWidth: 35, textColor: MUTED, fontSize: 6.5 }, 1: { cellWidth: 56 }, 2: { cellWidth: 35, textColor: MUTED, fontSize: 6.5 }, 3: { cellWidth: 56 } });

  doc.save(`EICR_${(job?.jobNumber || "report").replace(/\s+/g, "_")}_${(job?.client || "").replace(/\s+/g, "_")}.pdf`);
  } catch(err) {
    alert("PDF error: " + err.message);
    console.error("EICR PDF error:", err);
  }
}
...
.then(() => console.log("Test results saved"))
.catch(e => console.error("Test results save error:", e));
